...
public class DeviceScanActivity extends AppCompatActivity {
    ...
    private final ActivityResultLauncher<Intent> enableBluetoothLauncher = registerForActivityResult(
            ...
            result -> {
                if (result.getResultCode() == RESULT_OK) {
                    startBleScan(); // Start scan only if BT is enabled
                } else {
    ...
    private final ActivityResultLauncher<String[]> blePermissionsLauncher =
            registerForActivityResult(new ActivityResultContracts.RequestMultiplePermissions(), result -> {
                ...
                // Use a standard loop for compatibility with minSdk 23
                ...
                if (allGranted) {
                    // Permissions granted, now check BT state
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bleDeviceAdapter = new BleDeviceAdapter(this, device -> {
            ...
            if (ActivityCompat.checkSelfPermission(this, Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_GRANTED || Build.VERSION.SDK_INT < Build.VERSION_CODES.S) {
                ...
                Toast.makeText(this, "Connect permission missing to get name", Toast.LENGTH_SHORT).show();
            }

            setResult(Activity.RESULT_OK, resultIntent);
            finish(); // Go back to ChatActivity
        });
        ...
        scanButton.setOnClickListener(v -> {
            bleDeviceAdapter.clearDevices(); // Clear previous results
